...
class Graph{
    ...
    void topDownHeapify(int i, vector < pair <int,int> > &H){
    ...
    void dijkstra(int s){
        ...
        while(!minHeap.empty()){
            ...
            for( v = adj[u].begin(); v != adj[u].end(); v++){
                if(color[*v] == RED){
                    color[*v] = YELLOW;
                    pi[*v] = u;
                    priority[*v] = priority[u] + w[u][*v];
                    minHeap.push_back(make_pair(priority[*v],*v));
                    bottomUpHeapify(minHeap.size()-1,minHeap);
                }
                else if(color[*v] == YELLOW){
                    if(priority[*v] > priority[u] + w[u][*v]){
                        priority[*v] = priority[u] + w[u][*v];
                        decreaseKey(*v,priority[u] + w[u][*v],minHeap);
                        pi[*v] = u;
                    }
                }
            }
        ...
        for(int u = 0; u < n; u++){
            if(color[u] == RED){
                cout << "Boo" << endl;
                color[u] = YELLOW;
                minHeap.push_back(make_pair(priority[u],u));
                bottomUpHeapify(minHeap.size()-1,minHeap);
            }
        }
        while(!minHeap.empty()){
    ...
    void printPriority(int s){
        dijkstra(s);
        for(int i = 0; i < n; i++)
            cout << "Distance of " << i << " from " << s << " is " << priority[i] << endl;
        // for(int i = 0; i < n; i++)
        //     cout << "Predeessor of " << i << " is " << pi[i] << endl;
    }
